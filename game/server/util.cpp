...
void UTIL_Remove( IServerNetworkable *oldObj )
{
	...
	if ( pBaseEnt )
	{
#ifdef PORTAL_DLL //make sure entities are in the primary physics environment for the portal mod, this code should be safe even if the entity is in neither extra environment
...
static void SetMinMaxSize (CBaseEntity *pEnt, const Vector& mins, const Vector& maxs )
{
	for ( int i=0 ; i<3 ; i++ )
	{
		if ( mins[i] > maxs[i] )
		{
			DevWarning( "%s: backwards mins/maxs", ( pEnt ) ? pEnt->GetDebugName() : "<NULL>" );
			return;
		}
	}
...
CBaseEntity* UTIL_FindPickerEntity()
{
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		CBasePlayer* pPlayer = UTIL_PlayerByIndex(i);
		if (pPlayer)
		{
			CBaseEntity* pPickerEntity = pPlayer->FindPickerEntity();
			if ( pPickerEntity && pPickerEntity->IsPlayer() )
				return pPickerEntity;
		}
	}
