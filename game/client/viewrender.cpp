...
#ifdef PORTAL_DLL //in portal, we'd like a copy of the front buffer without the gun in it for use with the depth doubler
...
#ifdef PORTAL_DLL //in portal, we can't use the fog volume for the camera since it's almost never in the same fog volume as what's in front of the portal
	if( g_pPortalRender->GetViewRecursionLevel() == 0 )
	{
#endif
		if ( InVRRender() )
		{
			render->GetVisibleFogVolume( g_VRRenderer.m_viewOrigin, &fogVolumeInfo );
		}
		else
		{
...
#ifdef PORTAL_DLL //when rendering portal views point/plane intersections just don't cut it.
	if( g_pPortalRender->GetViewRecursionLevel() != 0 )
