...
struct PBR_Vars_t
{
    ...
    int useEnvAmbient;
    int specularTexture;
    int specularTextureNormal;
};


float GetLightMapScaleFactor( void )
{
...
BEGIN_VS_SHADER(PBR, "PBR shader")
    ...
    BEGIN_SHADER_PARAMS;
        ...
        // SHADER_PARAM(SELFILLUM, SHADER_PARAM_TYPE_BOOL, 0, "Emission");
        SHADER_PARAM(SELFILLUMMASK, SHADER_PARAM_TYPE_TEXTURE, "", "Emission texture");
        ...
        //SHADER_PARAM(NORMALMAPALPHAENVMAPMASK, SHADER_PARAM_TYPE_TEXTURE, "", "Specular F0 RGB map");
        //SHADER_PARAM(ENVMAPMASK, SHADER_PARAM_TYPE_TEXTURE, "", "Specular F0 RGB map");
    ...
    SHADER_INIT
    {
        ...
        if ( IS_FLAG_SET( MATERIAL_VAR_SELFILLUM ) && info.emissionTexture >= 0 && params[SELFILLUMMASK]->IsDefined())
        ...
        /*if (params[info.specularTextureNormal]->IsDefined())
        {
            LoadTexture(info.specularTextureNormal);
        }
        else*/ if (params[info.specularTexture]->IsDefined())
        {
    ...
    SHADER_DRAW
    {
        ...
        bool bHasEmissionTexture = IS_FLAG_SET( MATERIAL_VAR_SELFILLUM ) && (info.emissionTexture != -1) && params[info.emissionTexture]->IsTexture();
        ...
        /*if ( info.specularTextureNormal != -1 )
        {
            bHasSpecularTexture = (info.specularTextureNormal != -1) && params[info.specularTextureNormal]->IsTexture();
            specularTexture = info.specularTextureNormal;
        }
        else*/
        {
            bHasSpecularTexture = (info.specularTexture != -1) && params[info.specularTexture]->IsTexture();
            specularTexture = info.specularTexture;
        }
        ...
        else // Not snapshotting -- begin dynamic state
        {
            ...
            {
                BindTexture(SAMPLER_SPECULAR, specularTexture, 0);
            }
            else
            {
            ...
            float flLScale = GetLightMapScaleFactor();
